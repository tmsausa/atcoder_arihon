...
int main() {
    ...
    lint N, K; cin >> N >> K;
    vec<lint> a_list(N), b_list(N);
    range(i, 0, N) cin >> a_list.at(i);
    range(i, 0, N) cin >> b_list.at(i);
    sort(a_list.begin(), a_list.end());
    sort(b_list.begin(), b_list.end());
    auto count_num_less_than_or_equal_to = [&](lint k) {
        lint num = 0;
        for (lint a : a_list) {
            num += upper_bound(b_list.begin(), b_list.end(), k / a) - b_list.begin();
        }
        return num;
    };
    // (lo, hi]
    ...
    while (hi - lo > 1) {
        lint mid = lo + (hi - lo) / 2;
        if (count_num_less_than_or_equal_to(mid) < K) lo = mid;
        else hi = mid;
    }
    cout << hi << '\n';
}