...
int main() {
    ...
    range(i, 1, N) {
        lint ar = a_list.at(i);
        lint al = a_list.at(i - 1);
        if (al + ar > x) {
            lint num_to_reduce = al + ar - x;
            lint num_to_reduce_from_right = ar >= num_to_reduce ? num_to_reduce : ar;
            // reduce right first
            ans += num_to_reduce_from_right;
            a_list.at(i) -= num_to_reduce_from_right;
            // rest from left
            if (num_to_reduce > num_to_reduce_from_right) {
                ans += num_to_reduce - num_to_reduce_from_right;
                a_list.at(i - 1) -= num_to_reduce - num_to_reduce_from_right;
            }
        }
    }
    cout << ans << "\n";
