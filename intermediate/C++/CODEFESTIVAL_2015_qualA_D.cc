...
int main() {
    ...
    lint N, M; cin >> N >> M;
    vec<lint> X_list(M);
    range(i, 0, M) cin >> X_list.at(i);
    // (lo, hi]
    lint lo = max(X_list.at(0) - 1, N - X_list.at(M - 1)) - 1, hi = 2 * N;
    while (hi - lo > 1) {
        // cout << lo << " " << hi << "\n";
        lint mid = (lo + hi) / 2;
        bool possible = true;
        ...
        range(i, 0, M) {
            lint X = X_list.at(i);
            lint rest_left = X - D - 1;
            if (rest_left > mid) {
                possible = false;
                break;
            }
            lint R = max((mid - rest_left) / 2, mid - 2 * rest_left);
            if (i == M - 1) D = X + R;
            else D = min(X + R, X_list.at(i + 1) - 1);
            // cout << D << "\n";
        }
        if (D >= N) hi = mid;
        else lo = mid;
