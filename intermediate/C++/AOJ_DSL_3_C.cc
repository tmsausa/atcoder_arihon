...
int main() {
    ...
    lint N, Q; cin >> N >> Q;
    vec<lint> a_list(N), x_list(Q);
    range(i, 0, N) cin >> a_list.at(i);
    range(i, 0, Q) cin >> x_list.at(i);
    // cout << a_list << "\n";
    // cout << x_list << "\n";
    for (const auto& x : x_list) {
        ...
        lint cur_sum = 0;
        range(left, 0, N) {
            while (right < N and cur_sum + a_list.at(right) <= x) {
                cur_sum += a_list.at(right);
                right += 1;
            }
            count += right - left;
            if (left < right) cur_sum -= a_list.at(left);
            else right += 1;
        }
        cout << count << "\n";
    }
