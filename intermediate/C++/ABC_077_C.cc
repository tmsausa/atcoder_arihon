...
int main() {
    ...
    lint N; cin >> N;
    vec<lint> A_list(N), B_list(N), C_list(N);
    range(i, 0, N) cin >> A_list.at(i);
    range(i, 0, N) cin >> B_list.at(i);
    range(i, 0, N) cin >> C_list.at(i);
    sort(A_list.begin(), A_list.end());
    sort(B_list.begin(), B_list.end());
    sort(C_list.begin(), C_list.end());
    lint num_possibilities = 0;
    range(i, 0, N) {
        lint B = B_list.at(i);
        lint num_smaller_As = lower_bound(A_list.begin(), A_list.end(), B) - A_list.begin();
        lint num_larger_Cs = C_list.end() - upper_bound(C_list.begin(), C_list.end(), B);
        num_possibilities += num_smaller_As * num_larger_Cs;
        // cout << num_smaller_As << "\n";
        // cout << num_larger_Cs << "\n";
    }
    cout << num_possibilities << "\n";
}